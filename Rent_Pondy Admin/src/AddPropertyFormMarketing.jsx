...
function AddPropertyFormMarketing() {
  const location = useLocation();
   ...
   // Marketing-user login: stamp every submission with whoever's logged in,
   // so the "Added By" column on the admin Pre-Approved / Approved pages
   // reflects who entered the property. Persisted in localStorage so the
   // user only logs in once per browser.
   const [marketingUser, setMarketingUser] = useState(
     () => (typeof window !== "undefined" ? localStorage.getItem("marketingUserName") || "" : "")
   );
   const [showLoginModal, setShowLoginModal] = useState(!marketingUser);
   const [loginNameInput, setLoginNameInput] = useState("");

   useEffect(() => {
     if (!marketingUser) setShowLoginModal(true);
   }, [marketingUser]);

   const handleMarketingLogin = (e) => {
     if (e && typeof e.preventDefault === "function") e.preventDefault();
     const name = (loginNameInput || "").trim();
     if (!name) return;
     localStorage.setItem("marketingUserName", name);
     setMarketingUser(name);
     setLoginNameInput("");
     setShowLoginModal(false);
   };

   const handleMarketingLogout = () => {
     localStorage.removeItem("marketingUserName");
     setMarketingUser("");
...
: "N/A";
  ...
  // };
   const handlePreview = () => {
    // Mandatory-field validation removed: any submission lands in pre-approved.
    // Format checks below still run because invalid email/phone are not "missing"
    // values — they're malformed values that would otherwise be saved as-is.
...
"Your Property",
  ...
  const handleShowMore =async (e) => {
    ...
    // Mandatory step-field validation removed: users can advance any step regardless
    // of which fields are filled. Format checks below still run.
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    // 🔹 Stamp the property with the logged-in marketing user (Added By).
    // Backend only writes this on the very first save and preserves it on edits.
    if (marketingUser) {
      formDataToSend.append("addedBy", marketingUser);
    }
...
const resetForm = () => {
  // ✅ Clear rentId for new properties
  setRentId("");
  localStorage.removeItem("rentId");
  ...
  return (
    <div style={{
      ...
      {/* Login modal — auto-opens until a marketing user has been entered */}
      {showLoginModal && (
        <div
          style={{
            position: "fixed",
        ...
        >
          ...
          >
            <h3 style={{ margin: "0 0 8px", color: "#4F4B7E", fontWeight: 600 }}>
              Marketing Login
            </h3>
            <p style={{ margin: "0 0 18px", color: "#666", fontSize: 14 }}>
              Enter your name to start adding properties. Every property you add
              will be stamped with this name in the “Added By” column.
            </p>
            <input
              type="text"
              autoFocus
            ...
            <button
              ...
              style={{
                width: "100%",
                padding: "12px 16px",
                background: loginNameInput.trim() ? "#4F4B7E" : "#aaa",
                color: "#fff",
                border: "none",
                borderRadius: "8px",
                fontSize: 15,
                fontWeight: 600,
                cursor: loginNameInput.trim() ? "pointer" : "not-allowed",
              }}
            >
              Continue
            </button>
      ...
      {/* Small logged-in badge — lets a different person take over without DevTools */}
      {marketingUser && !showLoginModal && (
        <div
          style={{
            position: "fixed",
            top: 12,
        ...
        >
          <span style={{ color: "#4F4B7E", fontWeight: 600 }}>{marketingUser}</span>
          <button
            type="button"
            onClick={handleMarketingLogout}
            style={{
              background: "transparent",
              border: "none",
              color: "#dc3545",
    ...
    >
      ...
      <button
        onClick={() => {
          ...
          resetForm();
          // ✅ Navigate to clean URL without query params to prevent rentId persistence
          navigate('/dashboard/add-property-marketing', { replace: true });
        }}
        style={{
          backgroundColor: "#4CAF50",
          color: "white",
