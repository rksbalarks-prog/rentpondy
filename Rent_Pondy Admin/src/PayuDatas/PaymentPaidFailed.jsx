...
const PaymentPaidFailed = () => {
  ...
  const fetchPayments = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}/payments/pay-failed`
      );
  ...
  const handleDelete = async (id) => {
    if (!id) return;
    if (!window.confirm("Move this payment to Removed Payu? You can restore it later from the Removed Payu page."))
      return;
    try {
      await axios.delete(
        `${process.env.REACT_APP_API_URL}/payu-payment/${id}`
      );
  ...
  return (
    <div className="container mt-4">
      ...
      <h2 className="mb-4 text-center">Pay Failed Payments with Plan Details</h2>
...
export default PaymentPaidFailed;
