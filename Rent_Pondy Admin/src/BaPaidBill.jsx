...
const [filters, setFilters] = useState({
  ...
  try {
    const response = await axios.get(`${process.env.REACT_APP_API_URL}/buyer-bills/non-free-with-assistance-rent`);
  ...
  .map((item, i) => {
    ...
    return (
      <tr key={i}>
        ...
        <td>{prop.Ra_Id}</td>
        <td><PhoneCell phone={prop.phoneNumber} type="tenant" raId={prop.Ra_Id} /></td>
        ...
        <td>₹{Number(item.bill.netAmount ?? item.bill.billAmount ?? 0).toLocaleString('en-IN')}</td>
