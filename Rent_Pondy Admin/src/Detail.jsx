...
const Detail = ({ rentId: rentIdProp, phoneNumber: phoneNumberProp } = {}) => {
...
const [messageType, setMessageType] = React.useState("info"); // can be "error", "success", "info"
  ...
  // When rendered inside a popup modal these come in as props; when opened via
  // the /detail route they come from router state. Props take precedence.
  const locationState = location.state || {};
  const rentId = rentIdProp ?? locationState.rentId;
  const phoneNumber = phoneNumberProp ?? locationState.phoneNumber;
...
const handleSubmit = async ({ rentalAmount, rentId }) => {
  ...
  useEffect(() => {
    if (propertyDetails) {
      const phoneNumber = propertyDetails?.assignedPhoneNumber || propertyDetails?.phoneNumber || propertyDetails?.phone || '';
      if (phoneNumber) {
...
const handleOwnerContactClick = async () => {
  ...
  const contactNumber = propertyDetails?.assignedPhoneNumber || propertyDetails?.phoneNumber || propertyDetails?.phone || '';
