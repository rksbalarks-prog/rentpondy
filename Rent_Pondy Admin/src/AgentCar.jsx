...
const AgentProperty = () => {
  ...
  const fetchAllData = async () => {
    try {
      const [ agentRes] = await Promise.all([
        axios.get(`${process.env.REACT_APP_API_URL}/fetch-active-agent`),
       
      ]);

      setAgentList(agentRes.data.users);
  ...
  const adminRole = reduxAdminRole || localStorage.getItem("adminRole");
   ...
   const fileName = "Agent Property"; // current file
  ...
  <p>
      ...
      {/* Tables */}
      {renderTable(filterList(agentList), 'Agent Properties')}
      </div>
    </div>
  );
};
