...
const filteredPropertyData = propertyData.filter((item) => {
  ...
  const handleDelete = async (rentId) => {
    if (window.confirm(`Permanently delete RENT ID: ${rentId}? This cannot be undone.`)) {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/permanent-delete-property/${rentId}`, {
                method: 'DELETE',
            });
            ...
            if (response.ok) {
                // Remove the row from view permanently
                setPropertyData(prev => prev.filter(item => item.rentId !== rentId));
            }
        } catch (error) {
          ...
          {filteredPropertyData.map((item, index) => {
            ...
            return (
              <tr key={index}>
                ...
                <td className="sticky-col sticky-col-2"><PhoneCell phone={item.phoneNumber} type="owner" rentId={item.rentId} /></td>
