...
function EditForm() {
  ...
  const { rentId, phoneNumber } = location.state || {};
    ...
    // versa). Set when the loaded record's base disagrees with the admin's
    // scope; render a stop-screen instead of the form.
  ...
  useEffect(() => {
    const fetchPropertyData = async () => {
      try {
        ...
        // Block the edit if a PY/CH-scoped admin opened a record from the
        // other city — let them see what city it belongs to and stop.
...
const handleCombinedClick = async (e) => {
 ...
 const handleAnim = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Build a unified photoOrder array preserving the exact display order
    // (the default image is always moved to index 0 by handlePhotoSelect).
    // Existing photos -> their stored path; new uploads -> '__NEW__'
    // placeholders the backend swaps for the next uploaded file.
    // Only sent when photos exist, so a failed/empty photo load can never
    // wipe the property's saved photos.
    if (photos.length > 0) {
      const photoOrder = photos
        .map((photo) => {
          if (photo instanceof File || photo instanceof Blob) return '__NEW__';
    ...
    // Append new files in the same order so backend can match each '__NEW__'.
    ...
    // No new video uploaded — preserve the existing path so the route
    // doesn't drop it. New videos are sent separately below to keep them
    // from rolling back the rest of the save if the backend rejects them.
    ...
    if (videos && videos.length > 0 && videos[0] instanceof File) {
      ...
      if (hasVideoMime || hasVideoExt) {
        ...
        pendingVideo = new File([videoFile], videoFile.name, { type: cleanType });
      }
    ...
    try {
      ...
      if (pendingVideo) {
        ...
        videoForm.append("rentId", rentId);
        videoForm.append("video", pendingVideo);
        try {
          await axios.post(
            `${process.env.REACT_APP_API_URL}/update-rent-property`,
            videoForm,
    ...
    return (
      <div className="container py-5">
        <div className="alert alert-warning text-center shadow-sm" style={{ maxWidth: 560, margin: '40px auto', borderRadius: 12 }}>
          ...
          <p className="mb-2">
            This property is tagged <strong>{recordCity}</strong>, but you are
            signed in with the <strong>{myCity}</strong> city scope.
          </p>
