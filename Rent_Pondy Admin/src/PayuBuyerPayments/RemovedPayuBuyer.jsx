...
// Lists every PayU-buyer payment row that was soft-deleted from the
// Tenant Assistant Paid Failed / Pay Now / Pay Later pages. Records can
// be restored back into their original status table from here.
const RemovedPayuBuyer = () => {
  ...
  const fetchPayments = async () => {
    try {
      ...
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}/payu-buyer-removed`
      );
      setPayments(res.data.data || []);
    } catch (error) {
      console.error("Error fetching removed buyer payments:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRestore = async (id) => {
    ...
    try {
      await axios.post(
        `${process.env.REACT_APP_API_URL}/payu-buyer-payment/${id}/restore`
      );
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          <h3>Removed Payu Payments — Tenant Account</h3>
  ...
  return (
    <div className="container mt-4">
      ...
      <h2 className="mb-4 text-center">Removed Payu — Tenant Account</h2>
...
export default RemovedPayuBuyer;
