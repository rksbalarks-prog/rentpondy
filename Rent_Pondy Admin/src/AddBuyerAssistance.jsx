...
const handleConfirmSubmit = async () => {
  ...
  try {
    ...
    // Capture the logged-in admin name so the backend can persist it as `addedBy`.
    // The PUT route strips this field server-side to preserve the original creator.
    const loggedInAdminName = (typeof window !== 'undefined' && window.localStorage)
      ? (localStorage.getItem('adminName') || '').trim()
      : '';
    ...
    if (formData._id) {
      ...
      response = await axios.post(
        ...
        { ...formData, addedBy: loggedInAdminName }
      );
