...
// /process/rent-staff-report (registered as a top-level route in App.js,
// outside ProtectedDashboard). The original /process/dashboard/rent-staff-report
// stays untouched and continues to require login.
const RentStaffReportStandalone = () => {
  ...
  const fetchData = useCallback(async (rangeStart, rangeEnd) => {
    ...
    try {
      ...
      // Combine PROPERTY follow-ups (rentId) + TENANT follow-ups (Ra_Id) so the
      // Staff Daily Follow-up Summary and Hourly Activity tables count both.
      // Each row is tagged with a `_source` flag — the aggregator uses this to
      // pick the right ID field when classifying Follow-with-ID vs without-ID.
  ...
  const hourlyTotal = useMemo(() => {
    // Sum every numeric metric key across the hour buckets (covers combined
    // keys, the …_prop / …_ten splits and allFollowups). `hour` is the only
    // numeric non-metric field, so skip it.
    const acc = {};
  ...
  const handleExport = () => {
    ...
    // Render one metric cell from a bucket; split rows → "property / tenant".
  ...
  // ── Per-table exports ───────────────────────────────────────────────────────
  // Each writes a one-sheet workbook so the user can grab a single table without
  // the combined file produced by handleExport above.
  ...
  const handleExportSummary = () => {
    ...
    const ws = XLSX.utils.aoa_to_sheet([
      [`Rent Staff Report — Summary Table — ${rangeLabel}`],
      [],
      ...rows,
    ]);
    ws['!merges'] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 2 } }];
    ...
    saveAs(
      new Blob([buf], { type: 'application/octet-stream' }),
      `Rent_Staff_Report_Summary_${moment(appliedRange.start).format('YYYYMMDD')}_${moment(appliedRange.end).format('YYYYMMDD')}.xlsx`
    );
  };

  const handleExportDailyFollowUp = () => {
    const rangeLabel = `${moment(appliedRange.start).format('DD-MM-YYYY')} to ${moment(appliedRange.end).format('DD-MM-YYYY')}`;
    ...
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Staff Daily Follow-up');

    const buf = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    saveAs(
      new Blob([buf], { type: 'application/octet-stream' }),
      `Staff_Daily_FollowUp_${moment(appliedRange.start).format('YYYYMMDD')}_${moment(appliedRange.end).format('YYYYMMDD')}.xlsx`
    );
  };

  const handleExportHourly = () => {
    if (!hourlyStaff || !hourlyDate) {
      alert('Pick a staff member and a date before exporting the hourly table.');
      return;
    }

    const dateLabel = moment(hourlyDate).isValid()
    ...
    const dataColCount = hourlyBuckets.length; // 12 buckets
    const totalCols = 1 + dataColCount + 1;    // label + 12 hour columns + Total

    // Title row spans the whole sheet.
    const titleRow = new Array(totalCols).fill('');
    titleRow[0] = `Hourly Activity (9 AM – 9 PM) — ${hourlyStaff} on ${dateLabel}`;

    const nameRow = new Array(totalCols).fill('');
    nameRow[0] = 'Name';
    nameRow[1] = hourlyStaff;
    ...
    const wsAoa = [
      titleRow,
      new Array(totalCols).fill(''), // spacer
      nameRow,
      dateRow,
      hourRow,
      ...metricRows,
    ...
    // Merge title row across all columns; merge the value cells on Name/Date
    // rows so the staff/date span the hour columns like on screen.
    ws['!merges'] = [
      { s: { r: 0, c: 0 }, e: { r: 0, c: Math.max(0, totalCols - 1) } },
      { s: { r: 2, c: 1 }, e: { r: 2, c: Math.max(1, totalCols - 1) } },
      { s: { r: 3, c: 1 }, e: { r: 3, c: Math.max(1, totalCols - 1) } },
    ];

    ws['!cols'] = [
      { wch: 28 },
      ...Array(dataColCount).fill({ wch: 13 }),
      { wch: 14 },
    ];
    ...
    const buf = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    const fileDate = moment(hourlyDate).isValid()
      ? moment(hourlyDate).format('YYYYMMDD')
      : 'date';
    const fileStaff = String(hourlyStaff).replace(/[^A-Za-z0-9_-]+/g, '_');
    saveAs(
      new Blob([buf], { type: 'application/octet-stream' }),
      `Hourly_Activity_${fileStaff}_${fileDate}.xlsx`
    );
  };
    ...
    <div className="p-3" style={{ background: '#f9fafb', minHeight: '100vh' }}>
      ...
      >
        <div className="d-flex flex-wrap justify-content-between align-items-center mb-3" style={{ gap: '10px' }}>
          ...
          {/*
            Top "Export Excel" button removed — each table now has its own
            per-table export button (Summary Table, Staff Daily Follow-up,
            Hourly Activity). The `handleExport` function above still exists
            but is currently unused on this standalone page; left in place so
            it can be re-attached here if a combined workbook is needed later.
      ...
      {/*
        Stat-card sections (1. Login User Report … 6. Customer Care) hidden on
        the standalone page per request — only the two tables below should
        appear. The card components and `counts` data are still computed so
        the Summary Table and Excel export work unchanged. Uncomment the
        block to bring the cards back.
      ...
      */}
      ...
      <div style={{ ...sectionHeader, justifyContent: 'space-between' }}>
        <span><span role="img" aria-label="clipboard">📋</span> Summary Table</span>
        <button
          type="button"
          className="btn btn-success btn-sm d-flex align-items-center"
          ...
          disabled={loading}
          style={{ gap: '6px' }}
          title="Download just this table as Excel"
        >
          <FaFileExcel /> Export Excel
        </button>
      </div>
        ...
        <Table responsive bordered hover size="sm" className="mb-0">
          <thead style={{ background: '#f3f4f6' }}>
            <tr>
              ...
              {/* Action column hidden on the standalone page. */}
            </tr>
          </thead>
          <tbody>
            {(() => {
              ...
              const totalCols = 4 + dateCols.length; // #, Section, Report, Count + dates
                ...
                return (
                  <React.Fragment key={row.key}>
                    <tr>
                      ...
                      {/* Action cell hidden on the standalone page — drill-down expand disabled.
      ...
      <div style={{ ...sectionHeader, justifyContent: 'space-between' }}>
        <span>
          <FaUserFriends /> <span role="img" aria-label="phone">📞</span> Staff Daily Follow-up Summary
        </span>
        <button
          type="button"
          className="btn btn-success btn-sm d-flex align-items-center"
          onClick={handleExportDailyFollowUp}
          disabled={loading}
          style={{ gap: '6px' }}
          title="Download just this table as Excel"
        >
      ...
      <div style={{ ...sectionHeader, justifyContent: 'space-between' }}>
        <span>
          <FaPhone /> <span role="img" aria-label="clock">🕐</span> Hourly Activity (9 AM – 9 PM)
        </span>
        <button
          type="button"
          className="btn btn-success btn-sm d-flex align-items-center"
          onClick={handleExportHourly}
          disabled={loading || !hourlyStaff || !hourlyDate}
          style={{ gap: '6px' }}
          title={
            !hourlyStaff || !hourlyDate
              ? 'Pick a staff and date first'
              : 'Download just this table as Excel'
          }
        >
          <FaFileExcel /> Export Excel
        </button>
...
export default RentStaffReportStandalone;
