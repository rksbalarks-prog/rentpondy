...
const PendingProperties = () => {
  ...
  const [bulkUploadFilter, setBulkUploadFilter] = useState('all');
  ...
  const handleExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'PendingProperties');
    const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    const blob = new Blob([excelBuffer], { type: 'application/octet-stream' });
    saveAs(blob, `PendingProperties_${new Date().toISOString().slice(0,10)}.xlsx`);
  };
  ...
  useEffect(() => {
    handleSearch();
  }, [properties, rentIdSearch, startDate, endDate, phoneNumberSearch, followUpFilter, bulkUploadFilter]);
  ...
  // currently shown (one bill per property; billing activates them -> Approved).
    ...
    <div className="p-3">
      ...
      <form onSubmit={(e) => e.preventDefault()} className="d-flex flex-row gap-2 align-items-center flex-wrap">
        ...
        <select
          ...
          style={{ maxWidth: "150px" }}
        >
          <option value="all">All Bulk Upload</option>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((prop, idx) => (
              <tr key={prop._id} className={prop.status === "delete" ? 'table-danger' : ''}>
                ...
                <td className="sticky-col sticky-col-2"><PhoneCell phone={prop.phoneNumber} type="owner" rentId={prop.rentId} /></td>
