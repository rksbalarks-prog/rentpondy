...
const fs = require('fs');
const UploadImagesAds = require('../Ads/AdsModel'); // Adjust path as needed
const AdsImageClick = require('../Ads/AdsImageClickModel')
...
router.post('/uploadimage-ads', upload.array('images', 50), async (req, res) => {
    try {
        const imagePaths = req.files.map(file => file.path);
        const uploadEntry = new UploadImagesAds({
            images: imagePaths
        });
...
router.get('/get-uploadimages-ads', async (req, res) => {
    try {
        const allImages = await UploadImagesAds.find().sort({ uploadDate: -1 });
        res.status(200).json({
            success: true,
            data: allImages
        });
...
router.put('/edit-uploadimage-ads/:id', upload.array('images', 15), async (req, res) => {
    try {
        const { id } = req.params;
        const existing = await UploadImagesAds.findById(id);
        if (!existing) {
...
router.delete('/delete-uploadimage-ads/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const uploadDoc = await UploadImagesAds.findById(id);
        ...
        // Delete DB record
        await UploadImagesAds.findByIdAndDelete(id);
...
router.post('/record-image-click-ads', async (req, res) => {
  try {
    ...
    const clickEntry = new AdsImageClick({
      phoneNumber,
      imageUrl
    });
...
router.get('/get-image-clicks-ads', async (req, res) => {
  try {
    const clicks = await AdsImageClick.find().sort({ clickedAt: -1 });
