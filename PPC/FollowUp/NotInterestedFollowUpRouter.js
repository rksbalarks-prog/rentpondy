...
const NotInterestedFollowUp = require('../FollowUp/NotInterestedFollowUpModel');
...
// POST: Create a Not-Interested follow-up (phone-based)
router.post('/notinterested-followup-create', async (req, res) => {
  try {
    ...
    const existing = await NotInterestedFollowUp.findOne({
      phoneNumber,
      followupStatus,
      followupType,
    ...
    console.error('Create NotInterestedFollowUp Error:', err);
    console.error("Error Details:", {
      name: err.name,
      message: err.message,
      validationErrors: err.errors,
      // read back off req.body: the destructured consts above are scoped to the
      // try block and are not in scope here
      payload: req.body
    });
    res.status(500).json({ 
      success: false, 
...
// GET: All Not-Interested follow-ups
router.get('/notinterested-followup-list', async (req, res) => {
  try {
    ...
    console.error('List NotInterestedFollowUp Error:', err);
    res.status(500).json({ success: false, message: 'Server error' });
  }
});

// GET: Not-Interested follow-ups with today/past filter
router.get('/notinterested-followup-list-today-past', async (req, res) => {
  try {
    ...
    const followups = await NotInterestedFollowUp.find(filter).sort({ followupDate: -1, createdAt: -1 });
    res.status(200).json({ success: true, data: dedupe(followups) });
  } catch (err) {
    console.error('Today/Past NotInterestedFollowUp Error:', err);
    res.status(500).json({ success: false, message: 'Server error' });
  }
});

// PUT: Update a Not-Interested follow-up (Status, Type, Date, Admin Name, Remarks)
router.put('/notinterested-followup-update/:id', async (req, res) => {
  try {
    ...
    const followup = await NotInterestedFollowUp.findById(id);
    if (!followup) {
    ...
    console.error('Update NotInterestedFollowUp Error:', err);
    res.status(500).json({ success: false, message: 'Server error' });
  }
});
