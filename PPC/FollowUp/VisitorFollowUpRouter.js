...
const VisitorFollowUp = require('../FollowUp/VisitorFollowUpModel');
...
// POST: Create a Visitor follow-up (phone-based)
router.post('/visitor-followup-create', async (req, res) => {
  try {
    ...
    console.error('Create VisitorFollowUp Error:', err);
...
router.get('/visitor-followup-list', async (req, res) => {
  try {
    ...
    console.error('List VisitorFollowUp Error:', err);
    res.status(500).json({ success: false, message: 'Server error' });
  }
});

// GET: Visitor follow-ups with today/past filter
router.get('/visitor-followup-list-today-past', async (req, res) => {
  try {
    ...
    const followups = await VisitorFollowUp.find(filter).sort({ followupDate: -1, createdAt: -1 });
    res.status(200).json({ success: true, data: dedupe(followups) });
  } catch (err) {
    console.error('Today/Past VisitorFollowUp Error:', err);
    res.status(500).json({ success: false, message: 'Server error' });
  }
});

// PUT: Update a Visitor follow-up (Status, Type, Date, Admin Name, Remarks)
router.put('/visitor-followup-update/:id', async (req, res) => {
  try {
    ...
    const followup = await VisitorFollowUp.findById(id);
    if (!followup) {
    ...
    console.error('Update VisitorFollowUp Error:', err);
    res.status(500).json({ success: false, message: 'Server error' });
  }
});
