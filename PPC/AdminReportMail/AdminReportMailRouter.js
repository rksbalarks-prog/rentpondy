// Manual controls for the daily admin-report e-mail.
//
//   GET  /PPC/admin-report-mail/status
//   POST /PPC/admin-report-mail/send-now?dryRun=1   (x-report-token header)
//
// Same shape and guard as DataAddedMail's routes.
...
const { API_BASE } = require('./fetchReport');
...
router.get('/admin-report-mail/status', (req, res) => {
  res.json({
    success: true,
    ...
    manualTrigger: config.triggerToken ? 'enabled' : 'disabled (set ADMIN_REPORT_TOKEN)',
    scheduler: state.get(),
  });
});

router.post('/admin-report-mail/send-now', async (req, res) => {
  if (!config.triggerToken) {
    return res.status(503).json({
      success: false,
      message: 'Manual trigger disabled — set ADMIN_REPORT_TOKEN (or DATA_ADDED_REPORT_TOKEN) in .env',
    });
  }
  ...
  const result = await sendAdminReport({ to: to.length ? to : undefined, dryRun });
