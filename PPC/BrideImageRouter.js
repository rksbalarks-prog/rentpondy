...
const fs = require('fs');
const UploadImagesBride = require('./BrideImageModel'); // Adjust path as needed
const ImageClickBride = require('./ImageClickModelBride')
...
router.post('/uploadimage-bride', upload.array('images', 50), async (req, res) => {
    try {
        const imagePaths = req.files.map(file => file.path);
        const uploadEntry = new UploadImagesBride({
            images: imagePaths
        });
...
router.get('/get-uploadimages-bride', async (req, res) => {
    try {
        const allImages = await UploadImagesBride.find().sort({ uploadDate: -1 });
        res.status(200).json({
            success: true,
            data: allImages
        });
...
router.put('/edit-uploadimage-bride/:id', upload.array('images', 15), async (req, res) => {
    try {
        const { id } = req.params;
        const existing = await UploadImagesBride.findById(id);
        if (!existing) {
...
router.delete('/delete-uploadimage-bride/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const uploadDoc = await UploadImagesBride.findById(id);
        ...
        // Delete DB record
        await UploadImagesBride.findByIdAndDelete(id);
...
router.post('/record-image-click-bride', async (req, res) => {
  try {
    ...
    const clickEntry = new ImageClickBride({
      phoneNumber,
      imageUrl
    });
...
router.get('/get-image-clicks-bride', async (req, res) => {
  try {
    const clicks = await ImageClickBride.find().sort({ clickedAt: -1 });
