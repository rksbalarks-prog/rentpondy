...
const UpLoadDetailAds = () => {
  ...
  const fetchUploadedImages = async () => {
    try {
      const res = await axios.get(`${API_URL}/get-uploadimages-ads-detail`);
...
if (combinedFiles.length > 50) {
  ...
  const handleUpload = async () => {
    ...
    try {
      if (editId) {
        await axios.put(`${API_URL}/edit-uploadimage-ads-detail/${editId}`, formData);
        setMessage('Images updated successfully!');
        setEditId(null);
      } else {
        await axios.post(`${API_URL}/uploadimage-ads-detail`, formData);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API_URL}/delete-uploadimage-ads-detail/${id}`);
  ...
  return (
    <div style={{ padding: '20px' }}>
      <h2>{editId ? 'Edit Images' :  'Ads Images Upload (Maximum 50 Images)'}</h2>
  <div
      onClick={handleClick}
      style={{
    ...
    >
      ...
      <h2 className='text-center text-success m-3' > Get Ads Images </h2>
      {loading ? (
        <p>Loading...</p>
      ) : error ? (
              ...
              <tr key={upload._id}>
                ...
                <td style={{ display: 'flex', flexWrap: 'wrap' }}>
                  {upload.images.map((img, i) => (
                    ...
                    // src={`http://localhost:6006/${img.replace(/\\/g, '/')}`}
                      alt="upload"
...
export default UpLoadDetailAds;
