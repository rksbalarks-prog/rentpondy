    ...
    <div style={styles.overlay}>
      <div style={styles.modal}>
        ...
        onMouseOut={(e) => {
          ...
          <button style={styles.no} onClick={onClose}
             onMouseOver={(e) => {
       ...
       property.updatedAt && property.updatedAt !== property.createdAt
         ...
         : ` ${new Date(property.createdAt).toLocaleDateString('en-IN', {
             ...
             day: 'numeric'
                        ...
                        <h6 className="m-0">
                                   <span style={{ fontSize:'15px', color:'#4F4B7E', fontWeight:'bold', letterSpacing:"1px" }}> <FaRupeeSign className="me-2" color="#4F4B7E"/>
...
<span style={{ color: '#4F4B7E', fontSize: '13px', marginLeft: "5px", fontSize: '11px' }}>
  ...
  style={{ color: "#4F4B7E", margin: "0px", cursor: "pointer" }}
>
  {finalContactNumber ? (
    <div className="mt-2">
      <img
...
<p>No properties found.</p>
    ...
    <div className="row m-0 w-100">
      {properties.map((property) => (
        <PropertyCard key={property.rentId} property={property} onRemove={onRemove} onUndo={onUndo} />
      ))}
    </div>
  );
};
...
//   try {
//     setLoading(true);
//     const apiUrl = `${process.env.REACT_APP_API_URL}/get-favorite-owner-rent`;

//     const { data } = await axios.get(apiUrl, { params: { phoneNumber } });

//     // 🔁 Sort properties: New → Old
//     // const sortedFavorites = data.favoriteRequestsData.sort(
...
const fetchInterestedProperties = useCallback(async () => {
  ...
  try {
    ...
    const favoriteRequests = Array.isArray(data.favoriteRequestsData)
      ? data.favoriteRequestsData
      : [];

    // 🔁 Enrich each property with propertyMessage using rentId
    const enrichedFavorites = await Promise.all(
      favoriteRequests.map(async (property) => {
    ...
    const sortedFavorites = enrichedFavorites.sort(
      (a, b) => new Date(b.updatedAt || b.createdAt) - new Date(a.updatedAt || a.createdAt)
    );
...
useEffect(() => {
  ...
  return (
    ...
    >
      ...
      </button> <h3 className="m-0" style={{fontSize:"18px"}}>MY SHORTLIST PROPERTY</h3> </div>
