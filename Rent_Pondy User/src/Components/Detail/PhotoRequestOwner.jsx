...
//     try {
//       const response = await axios.get(`${process.env.REACT_APP_API_URL}/photo-requests-rent/owner/${phoneNumber}`);
...
useEffect(() => {
  const fetchProperties = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const sortedProperties = enriched.sort(
          (a, b) => new Date(b.updatedAt || b.createdAt) - new Date(a.updatedAt || a.createdAt)
        );
  ...
  const handleRemoveProperty = async (rentId) => {
    ...
    try {
      const response = await axios.put(`${process.env.REACT_APP_API_URL}/photo-requests-rent/delete/${rentId}`);
      if (response.status === 200) {
  ...
  const handleUndoRemove = async (rentId) => {
    ...
    try {
      const response = await axios.put(`${process.env.REACT_APP_API_URL}/photo-requests-rent/undo/${rentId}`);
      if (response.status === 200) {
        ...
        if (restoredProperty) {
          ...
          setRemovedProperties((prevRemovedProperties) =>
            prevRemovedProperties.filter((property) => property.rentId !== rentId)
          );
  
          localStorage.setItem(
            'removedProperties',
            JSON.stringify(removedProperties.filter((property) => property.rentId !== rentId))
          );
        }
      }
    } catch (error) {
