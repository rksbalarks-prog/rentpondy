...
import {  FaArrowLeft, FaCalendarAlt, FaChevronLeft } from "react-icons/fa";
...
const HelpRequests = () => {
  ...
  const handleContactCall = async (rentId, phoneNumber) => {
    try {
  ...
  const handleRemoveHelpRequest = async (rentId, requesterPhoneNumber) => {
    confirmAction("Are you sure you want to remove this help request?", async () => {
      try {
        await axios.put(`${process.env.REACT_APP_API_URL}/help/delete/${rentId}/${requesterPhoneNumber}`);
        ...
        setRemovedHelpRequests([...removedHelpRequests, { rentId, requesterPhoneNumber }]);
  ...
  const handleUndoHelpRequest = async (rentId, requesterPhoneNumber) => {
    confirmAction("Do you want to restore this help request?", async () => {
      try {
        const response = await axios.put(`${process.env.REACT_APP_API_URL}/help/undo/${rentId}/${requesterPhoneNumber}`);
...
const navigate = useNavigate();
  ...
  return (
    ...
    >
      ...
      </button> <h3 className="m-0" style={{fontSize:"18px"}}>NEEDHELP OWNER</h3> </div>
...
<button
  ...
  onClick={async (e) => {
    e.stopPropagation();handleContactCall(property.rentId, user);
    window.location.href = `tel:${user}`;
  }}
>
  Call
                      ...
                      <button
                        ...
                        onClick={(e) =>{e.stopPropagation();
                           handleRemoveHelpRequest(property.rentId, user)}}
                      >
                        Remove
                      </button>
                    </div>
        ...
        removedHelpRequests.length > 0 ? (
          ...
          >
            <div className="row d-flex align-items-center">
              ...
              <div className='p-0' style={{background:"#707070", width:"2px", height:"80px"}}></div>
              <div className="col-7 p-0 ms-4">
                <div className='text-center rounded-1 w-100 mb-1' style={{border:"2px solid #4F4B7E", color:"#4F4B7E", fontSize:"14px"}}>HELP REQUESTED </div>
                <div className="d-flex">
                  <p className="mb-1" style={{ color: "#474747", fontWeight: "500",fontSize:"12px" }}>
                  PUC- {property.rentId}
                  </p>
          ...
          {showFullNumber
            ?  <div className="d-flex justify-content-between align-items-center ps-2 pe-2 mt-1">
                  ...
                  <button className="btn text-white px-3 py-1 flex-grow-1 mx-1"
                    ...
                    onClick={(e) =>{e.stopPropagation();
                     handleUndoHelpRequest(property.rentId, property.requesterPhoneNumber)}}> Undo</button>
