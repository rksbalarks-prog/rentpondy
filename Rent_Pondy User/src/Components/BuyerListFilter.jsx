...
//   // ✅ FIXED: Better phone number handling with fallbacks
...
//  // Function to record view when "More" button is clicked
...
//         Ra_Id: Ra_Id,
//         viewedBy: 'User', // You can customize this
//         viewedByPhoneNumber: phoneNumber
//       });
...
//   // Combined function for navigation and recording view
...
//        </button> <h3 className="m-0 " style={{fontSize:"18px"}}>Tenant List </h3> </div>
...
//         style={{ background: "#4F4B7E", fontSize: "13px" }}
//         onMouseOver={(e) => {
//           e.target.style.background = "#CDC9F9";
...
const BuyerLists = () => {
  ...
  // ✅ FIXED: Function to record view when "More" button is clicked
  // const handleRecordView = async (Ra_Id) => {
  //   try {
  //     setLoadingView(true);
  ...
  //       Ra_Id: Ra_Id,
  //       viewedBy: 'User',
  //       viewedByPhoneNumber: currentPhoneNumber
  //     });
...
const handleRecordView = async (Ra_Id) => {
  try {
    ...
    const response = await axios.post(`${process.env.REACT_APP_API_URL}/record-buyer-assist-view`, {
      Ra_Id: Ra_Id,
    ...
    setMessage('Failed to record view. Please try again.');
  } finally {
    setLoadingView(false);
  }
};
...
const sendTenantVisitNotification = async (tenantData, userName, messageType = "visit") => {
  try {
    const tenantPhone = tenantData?.phoneNumber || "";
    const raId = tenantData?.Ra_Id || "N/A";
    const location = tenantData?.area || tenantData?.state || "BHARATHIPURAM";
    const ownerName = tenantData?.raName || "Tenant";
    const ownerPhone = tenantData?.phoneNumber || "N/A";

    console.log("📱 Starting notification...", messageType);
    ...
    console.log("⚠️ Error in tenant visit notification:", error.message);
  }
};

  // ✅ FIXED: Combined function for navigation and recording view
  const handleMoreClick = async (Ra_Id, tenantCard) => {
    // First record the view
    await handleRecordView(Ra_Id);
    
    // Send WhatsApp notifications to both user and tenant
    const currentUserName = localStorage.getItem("userName") || "Guest User";
    await sendTenantVisitNotification(tenantCard, currentUserName);
  ...
  const handleSendInterest = async (id, tenantCard) => {
    try {
      const currentPhoneNumber = getPhoneNumber();
      ...
      if (response.ok) {
        ...
        // Send WhatsApp notifications to both user and tenant
        const currentUserName = localStorage.getItem("userName") || "Guest User";
        await sendTenantVisitNotification(tenantCard, currentUserName, "interest");
      } else {
        setMessage(`Failed to send interest: ${data.message}`);
      }
    } catch (error) {
    ...
    const fetchAllAssistanceData = async () => {
      try {
        const assistanceResponse = await axios.get(
          `${process.env.REACT_APP_API_URL}/get-buyerAssistances-rent`
        );
        const sortedAssistanceData = assistanceResponse.data.data.sort(
          (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
        );
    ...
    <div className="container d-flex align-items-center justify-content-center p-0">
      <div className="d-flex flex-column align-items-center justify-content-center m-0" 
        ...
        <div className="d-flex align-items-center justify-content-start w-100 p-2" 
          ...
          <h3 className="m-0" style={{ fontSize: "18px" }}>Tenant List</h3>
        </div>
        ...
        <div className="row g-2 w-100">
          ...
          >
            ...
            {/* Messages */}
            {message && (
              <div className="alert alert-success text-center w-100" style={{ fontSize: "14px" }}>
                {message}
              </div>
            )}
            ...
            {assistanceData.length > 0 ? (
                  ...
                  <div className="p-1">
                    ...
                    <div className="d-flex justify-content-end align-items-center m-0 mt-2">
                      <button
                        ...
                        onClick={() => handleSendInterest(card._id, card)}
                      >
                        Send Interest
                      </button>
                      ...
                      <button
                        ...
                        onClick={() => handleMoreClick(card.Ra_Id, card)}
                        disabled={loadingView}
                      >
                        {loadingView ? "Loading..." : "More"}
                      </button>
              ...
              <div className="text-center my-4"
                ...
                <p>No tenant assistance requests found.</p>
              </div>
            )}
