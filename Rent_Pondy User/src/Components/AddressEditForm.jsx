...
function EditAddress() {
  ...
  // Get rentId and phoneNumber from location.state or fallback to localStorage
  const rentIdFromState = location.state?.rentId || "";
  const phoneNumberFromState = location.state?.phoneNumber || localStorage.getItem("phoneNumber") || "";

  const [rentId] = useState(rentIdFromState);
  const [phoneNumber] = useState(phoneNumberFromState);
...
const isReadOnly = true; // set true to make it readonly
 ...
     </button><h3 className="m-0" style={{fontSize:"18px"}}>EDIT PROPERTY ADDRESS</h3> </div>
