...
//             `${process.env.REACT_APP_API_URL}/fetch-owner-matched-properties-rent?phoneNumber=${storedPhoneNumber}`
//           );
...
//             `${process.env.REACT_APP_API_URL}/get-buyerAssistance?phoneNumber=${storedPhoneNumber}`
//           );
...
//       );
//       setMessage("Tenant Assistance request removed successfully!");
//       setTimeout(() => navigate(-1), 1000);
//     } catch (error) {
//       setMessage("Failed to delete Tenant Assistance request.");
//     }
//   };
...
//            </button>
//           <h3 className="m-0" style={{ fontSize: "18px", fontWeight: "bold" }}>
//             DETAILED TENANT ASSISTANT
//           </h3>
//         </div>
...
//             <h5 className='ps-3 ms-3' style={{ color: "#4F4B7E", fontWeight: "bold", marginBottom: "10px", fontSize: "15px" }}>
//               Tenant Profile
//             </h5>
...
// >
//   {requestData.raName && requestData.raName.trim() !== "" ? requestData.raName : "Tenant"}
// </span>
...
//                   <div>
//                     <h6 className="m-0 text-muted" style={{ fontSize: "12px" }}>Tenant Phone Number</h6>
...
export default function DetailBuyerAssis() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // ✅ FIXED: Fetch plan details and filter for current Ra_Id
        if (storedPhoneNumber) {
          const planResponse = await axios.get(
            `${process.env.REACT_APP_API_URL}/get-buyerAssistance?phoneNumber=${storedPhoneNumber}`
          );
          
          // Find the plan that matches the current Ra_Id
          const currentRaPlan = planResponse.data.data.find(
            item => item.Ra_Id === parseInt(Ra_Id)
          );
          
          if (currentRaPlan) {
            setPlanDetails(currentRaPlan.planDetails);
            
            // Calculate days remaining
            const daysRemaining = calculateDaysRemaining(currentRaPlan.planDetails.planExpiryDate);
            setDaysRemaining(daysRemaining);
          } else {
            // If no specific plan found, use the first one or set default
            setPlanDetails({
  ...
  // Function to get status message based on days remaining
  const getPlanStatusMessage = () => {
    if (daysRemaining === null) {
      return planDetails?.paymentStatus || "Not Active";
    }
  ...
  const getPlanStatusColor = () => {
    ...
    if (daysRemaining < 0) {
      ...
      return "#FF6B00"; // Orange for critical (1-3 days)
    } else if (daysRemaining <= 10) {
      return "#FFA500"; // Orange for warning (4-10 days)
    } else {
      return "green"; // Green for safe (more than 10 days)
    }
  };
  ...
  // Function to check if plan is expiring soon (within 10 days)
  ...
  const handleMatchClick = () => {
    if (matchedProperties.length > 0) {
      openConfirm("match", {
        phoneNumber: matchedProperties[0].phoneNumber,
        rentId: matchedProperties[0]._id,
      });
    } else {
      setNoMatchMessage("There are no matched properties");
    }
  ...
  return (
    <div className='d-flex justify-content-center align-items-center w-100'>
      <div className='d-flex flex-column' style={{ maxWidth: "500px", width: "100%" }}>
        ...
        {isPlanExpired() && (
          <div className="alert alert-danger text-center m-2" style={{ borderRadius: '8px' }}>
            <strong>⚠️ Plan Expired</strong> - Please renew your plan to continue using tenant assistance services.
          </div>
        )}

        {isPlanExpiringSoon() && (
          <div className="alert alert-warning text-center m-2" style={{ borderRadius: '8px' }}>
            <strong>⚠️ Plan Expiring Soon</strong> - Your plan will expire in {daysRemaining} day{daysRemaining > 1 ? 's' : ''}. Please renew to avoid interruption.
          </div>
        )}
        ...
        <div className="d-flex justify-content-center w-100">
          <div className='row w-100 mt-3 p-0'>
            ...
            {/* ✅ UPDATED: Plan Details Section with Countdown */}
            <h5 className='ps-3 ms-3' style={{ color: "#4F4B7E", fontWeight: "bold", marginBottom: "10px", fontSize: "15px" }}>
              Plan Details
            </h5>
              ...
              <div className="d-flex flex-row align-items-start w-100 ps-3">
                <div className="d-flex align-items-center col-6">
                  ...
                  <div>
                    <h6 className="m-0 text-muted" style={{ fontSize: "12px" }}>Plan Name</h6>
                    <span className="card-text" style={{ color: "#1D1D1D", fontWeight: "500", fontSize: "14px" }}>
                      {planDetails?.planName || "No Active Plan"}
                    </span>
                  </div>
                </div>
            ...
            {/* Plan Status with Countdown */}
            <div className="d-flex align-items-center mb-3">
              <div className="d-flex flex-row align-items-start w-100 ps-3">
                <div className="d-flex align-items-center col-6">
                  <div>
                    <h6 className="m-0 text-muted" style={{ fontSize: "12px" }}>Plan Status</h6>
                    <span 
                      className="card-text" 
                      style={{ 
                ...
                <div className="d-flex align-items-center col-6">
                  <div>
                    <h6 className="m-0 text-muted" style={{ fontSize: "12px" }}>Plan Duration</h6>
                    <span className="card-text" style={{ color: "#1D1D1D", fontWeight: "500", fontSize: "14px" }}>
                      {planDetails?.durationDays ? `${planDetails.durationDays} days` : "N/A"}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="d-flex align-items-center mb-3">
                <div className="d-flex flex-row align-items-start w-100 ps-3">
                  <div className="d-flex align-items-center col-12">
                    <div>
                      <h6 className="m-0 text-muted" style={{ fontSize: "12px" }}>Days Remaining</h6>
                      <span 
                        className="card-text" 
                        style={{ 
                          color: getPlanStatusColor(), 
            ...
            <div className="d-flex justify-content-between align-items-center ps-2 pe-2 mt-5 mb-5 col-12">
              ...
              <button
                ...
                className="btn text-white px-3 py-1 mx-1"
                style={{ background: "#4F4B7E", fontSize: "13px" }}
                disabled={isPlanExpired()} // Disable if plan expired
              >
                Match Prop
              </button>
            ...
            {/* Renew Plan Button if expired or expiring soon */}
            {(isPlanExpired() || isPlanExpiringSoon()) && (
              <div className="d-flex justify-content-center mb-3">
                <button
                  className="btn text-white px-4 py-2"
                  style={{ background: "#28a745", fontSize: "14px", fontWeight: "bold" }}
                  onClick={() => navigate("/buyer-plan")}
                >
                  {isPlanExpired() ? "Renew Expired Plan" : "Renew Plan Now"}
                </button>
              </div>
            )}
...
//                       className="card-text" 
//                       style={{ 
//                         color: planDetails?.isActive ? "green" : "red", 
//                         fontWeight: "500", 
//                         fontSize: "14px" 
//                       }}
//                     >
//                       {planDetails?.paymentStatus || "Not Active"}
//                     </span>
//                   </div>
//                 </div>
