...
const Bride = () => {
  ...
  const fetchUploadedImages = async () => {
    try {
      const res = await axios.get(`https://ppcpondy.com/PPC/PPC/get-uploadimages-bride`);
  ...
  // Record user viewing the Bride page
  useEffect(() => {
  ...
  // Handle image click and record it in DB
  const handleImageClick = async (imageUrl) => {
    try {
      if (!phoneNumber) return;

      await axios.post(`https://ppcpondy.com/PPC/PPC/record-image-click-bride`, {
        phoneNumber,
        imageUrl
      });
    } catch (err) {
      console.error("Failed to record image click:", err);
    }
        setShowPopup(true);
