...
import rangoli from '../Assets/bgdesignbottom.png';
import 'bootstrap/dist/css/bootstrap.min.css';
...
const MoreComponent = () => {
  ...
  // Save tab to localStorage on tab click
...
useEffect(() => {
  const fetchPhotoRequestCount = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/photo-requests/owner/count/${phoneNumber}`);
      setOwnerPhotoRequestCount(res.data.photoRequestCount || 0);
    } catch (err) {
      console.error("Error fetching photo request count:", err);
      setOwnerPhotoRequestCount(0);
    }
  };

  if (phoneNumber) {
    fetchPhotoRequestCount();
  }
}, [phoneNumber]);
...
//       if (res.data.ownerPhotoRequestCount !== undefined) {
//         setOwnerPhotoRequestCount(res.data.ownerPhotoRequestCount || 0);
//       }
//     } catch (err) {
//       console.error("Failed to fetch buyer photo request count.");
//     }
...
useEffect(() => {
    const fetchContactBuyerCount = async () => {
      try {
        ...
        if (res.data.contactBuyerCount !== undefined) {
          setContactBuyerCount(res.data.contactBuyerCount || 0);
          console.log("Contact Buyer Count:", res.data.contactBuyerCount);
        }
      } catch (error) {
        console.error("Failed to fetch contact buyer count:", error);
        setContactBuyerCount(0);
      }
    };
...
useEffect(() => {
 ...
 fontFamily: "Inter, sans-serif",
  ...
  <div
className="d-flex align-items-center justify-content-start w-100 p-2"           style={{
        background: "#EFEFEF",
        position: "sticky",
        top: 0,
        zIndex: 1000,
  ...
  >
    ...
    <h3 className="m-0" style={{fontSize:"18px"}}>More</h3>
  </div>
        ...
        <div style={{
            ...
            <ul className="list-group custom-list-group" >
              ...
              fontSize: '16px',
                    ...
                    >
        <span style={{fontSize:"14px" , fontWeight:400 , color:"#5E5E5E" , fontWeight:500}}>ADD PROPERTY</span>      
                    </li> */}
