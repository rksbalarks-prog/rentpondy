...
const formatPrice = (rentalAmount) => {
  ...
  //   try {
  //     const response = await axios.get(`${process.env.REACT_APP_API_URL}/expired-plans`);
  //     if (response.status === 200) {
  //       const allExpiredPlans = response.data.expiredPlans;

  //       // Filter by phoneNumber (if passed as prop)
  //       const filteredPlans = phoneNumber
  //         ? allExpiredPlans.filter(plan => plan.phone === phoneNumber)
  //         : allExpiredPlans;

  //       setProperties(filteredPlans);
  //     }
  //   } catch (error) {
  //     console.error('Failed to fetch expired plans:', error);
  //   } finally {
  //     setLoading(false);
  //   }
  // };
  ...
  return (
    <div className="container d-flex align-items-center justify-content-center p-0">
     ...
     >
       ...
       </button>          <h3 className="m-0 " style={{ fontSize: "18px" }}>Expired Property</h3>
        </div> 
