...
const UpLoadImagesGroom = () => {
  ...
  const fetchUploadedImages = async () => {
    try {
      const res = await axios.get(`${API_URL}/get-uploadimages`);
...
const handleFileChange = (e) => {
 ...
 if (combinedFiles.length > 50) {
  ...
  const handleUpload = async () => {
    ...
    try {
      if (editId) {
        await axios.put(`${API_URL}/edit-uploadimage/${editId}`, formData);
        setMessage('Images updated successfully!');
        setEditId(null);
      } else {
        await axios.post(`${API_URL}/uploadimage`, formData);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API_URL}/delete-uploadimage/${id}`);
  ...
  return (
    <div style={{ padding: '20px' }}>
      <h2>{editId ? 'Edit Images' : 'Groom Images Upload (Maximum 50 Images)'}</h2>
  <div
      onClick={handleClick}
      style={{
    ...
    >
      ...
      <h2 className='text-center text-success m-3' > Get Groom Images </h2>
      {loading ? (
        <p>Loading...</p>
      ) : error ? (
...
export default UpLoadImagesGroom;
